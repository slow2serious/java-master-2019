...
public class IterativeParallelism implements ListIP {
    ...
    private <T, R> R baseTask(int threads, final List<? extends T> values,
        ...
        ConcurrentUtils.checkThreads(threads);
        ...
        if (mapper != null) {
            ...
            for (int i = 0; i < threads; i++) {
                final int pos = i;
                ConcurrentUtils.addAndStart(workers, new Thread(() -> res.set(pos, task.apply(subTasks.get(pos)))));
            }
            ConcurrentUtils.joinThreads(workers);
        }
